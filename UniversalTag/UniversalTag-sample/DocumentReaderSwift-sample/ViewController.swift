...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var externalDeviceSwitch: UISwitch!
      
    var imagePicker = UIImagePickerController()
    
    private var selectedScenario: String?
    private var needConnectToExternalDevice: Bool = true
    
    lazy var pinPadTagAdapter: DeviceAdapter = {
        return DeviceAdapter()
    }()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        statusView.setStatus(.initialization)
        initializationReader()
    }
    
    @IBAction private func readRFIDSwitchChanged(_ sender: UISwitch) {
        self.externalDeviceSwitch.isEnabled = sender.isOn
    }
    
    @IBAction private func externalDeviceSwitchChanged(_ sender: UISwitch) {
        self.needConnectToExternalDevice = sender.isOn
    }
    ...
    func initializationReader() {
        ...
        DocReader.shared.initializeReader(config: config) { (success, error) in
            DispatchQueue.main.async {
                if success {
                    self.statusView.setStatus(.empty)
                    ...
                    self.statusView.setStatus(.initializationError(error?.localizedDescription ?? "unknown"))
    ...
    func startRFIDReading() {
        if needConnectToExternalDevice {
            pickerView.isHidden = true
            statusView.setStatus(.connectingToDevice)
            
            ///** Connecting to the external Bluetooth device and waiting for connected tag
            pinPadTagAdapter.tagReadyConnectionCompletion = { [weak self] universalTag in
                guard let self = self else { return }
                self.statusView.setStatus(.connectingToNFC)
                
                DocReader.shared.readRFID(universalNFCTag: universalTag) { notificationAction, notification in
                    
                } completion: { [weak self]  action, results, error, errorCode in
                    guard let self = self else { return }
                    ...
                    default:
                        print("Action - unknown, error: \(error?.localizedDescription ?? "")")
                        self.nameLabel.text = error?.localizedDescription
                    }
                }
            }
        } else {
            ///** Starting RFIDReader (Native | 1120)
